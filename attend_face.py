							...
							for i in range(row):
								if(id==int(face_id[i])):
									...
									if((str(id)) not in dict):
										...
										myresult = mycursor.fetchone()
										if int(myresult[0]) < 1:
											sql = "INSERT INTO attendance (attend_date, attend_time, is_late, deduction, is_present, face_id, created_at) VALUES (%s, %s, %s,%s, %s, %s, %s)"
											val = (attend_date, attend_time , int(is_late), int(total_deduction), 1, face_id[i], dt_object)
											mycursor.execute(sql, val)
											mydb.commit()
											playsound('success.wav')
											print("[INFO] " + first_name[i] + ", your attend time has been recorded in database")
										else:
											print("[INFO] " + str(first_name[i]) + ", your attend time already recorded in database")
